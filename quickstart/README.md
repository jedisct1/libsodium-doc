...
* `A` and `B` both call `crypto_kx_keypair()` to create their own key pair.
  Secret keys must remain secret, but `A` can send its public key to `B` or
...
Remember that public keys for both operations are very small: only 32 bytes. Concatenating both produces an aggregate public key that is only 64 bytes long. For most applications, the overhead is negligible, and conversions are not required any longer.
...
* If public verifiability is required, and the same message can be decrypted by multiple recipients, encrypt `(H(sender_id || message) || message)` (if using AES-GCM or Salsa/ChaCha-Poly1305), then sign the ciphertext.
