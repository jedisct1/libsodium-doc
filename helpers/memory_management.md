...
Therefore, if cold boot attacks or at-rest data protection are serious concerns for your threat model, the most effective defense is to encrypt the entire disk volume and encrypt the swap partition (or disable swap entirely). Additionally, hibernation should be completely disabled on systems running sensitive applications. Hibernation is a system-wide operation that writes the entire contents of RAM to disk, including all processes, and there is no way for individual processes to opt-out or prevent their memory from being included in the hibernation snapshot. This completely bypasses all memory locking protections. Encryption at rest ensures that even if memory contents are written to disk, they remain protected. Memory locking should be viewed as a defense-in-depth measure, not a complete solution for preventing sensitive data from ever reaching persistent storage.
